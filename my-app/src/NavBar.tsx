...
class NavBarView extends React.Component<IBaseProps, any> {
    ...
    dynamiNavBar = (data: Array<IDataElementBar>) => {
        data.map((item: IDataElementBar) => {
            ...
            if (item.hasChild) {
                return(
                    <div className="nav-dropdown">
                        <button className="nav-dropdown-btn"><Link className="nav-dropdown-btn-a" to={item.path}/>${item.title}</button>
                        {this.dynamiNavBar(item.child)}
                    </div>
                )
            }
            else {
                return(
                    <Link className="nav-dropdown-content-NavLink" to={item.path}>${item.title}</Link>
                )
            }
        })
    }
    ...
    render() {
        return(
            <div className="nav-container">
                ...
                {/* <div className="nav-dropdown" id="nav-dropdown-1" >
